...
fn process_wad_file(
    ...
    if ctx.relocate_combo_bins || selected_fixes.iter().any(|f| f == "combo_bin_relocate") {
        if dry_run {
            ...
            combo_bins_relocated = hematite_orchestrate::fix_folder::run_combo_bin_relocate(
                &mut all_files,
                ctx.game_wad,
                live,
            );
    ...
    // processed and downstream pipeline steps can iterate over them.
    {
        ...
        if seeds.is_empty() {
            ...
            if let Some(live) = live {
                hematite_orchestrate::fix_folder::prime_champion_wads(
                    live,
                    champions,
                    unique_champs.iter().copied(),
                );
            }
        }
    }
    ...
    if ctx.restore_anm {
        if dry_run {
            ...
            let restored = hematite_orchestrate::fix_folder::run_restore_anm(
                &mut all_files,
                &bin_provider,
                ctx.game_wad,
                live,
    ...
    if let Some(opts) = repath_opts {
        if !dry_run {
            ...
            if let Some(ref game_wad_path) = opts.game_wad {
                game_files_added = hematite_orchestrate::fix_folder::extract_missing_from_game_wad(
                    game_wad_path,
                    &mut all_files,
                    &bin_provider,
                ...
                game_files_added = hematite_orchestrate::fix_folder::extract_missing_from_live(
                    live,
                    &mut all_files,
                    &bin_provider,
                    hash_provider.as_ref(),
...
/// Process a `.wad.client` folder.
///
/// Thin adapter: builds a library-neutral [`FixOptions`] + a
/// `UiReporter`-backed `ProgressSink` from the CLI `ProcessContext`, then
/// delegates the whole extract → detect → fix → recover → rebuild pipeline to
/// [`hematite_orchestrate::fix_folder`]. Single source of truth — the CLI and
/// Flint drive the same code.
fn process_wad_folder(
    ...
    let opts = hematite_orchestrate::FixOptions {
        ...
        // skips every write, exactly as the pre-lift folder path did.
        dry_run: ctx.dry_run,
        detect_only: ctx.check,
        repath: ctx.repath_opts,
        restore_anm: ctx.restore_anm,
